...
def analyze_step_definitions(project_path, results):
    # Initialize unimplemented steps counter
    unimplemented_steps = 0

    # Look for step definition files in stepDefinitions folder
    step_def_paths = [
    ...
    # Patterns to identify unimplemented steps
    unimplemented_patterns = [
        r'@\w+\([^)]*\)\s*public\s+void\s+\w+\([^)]*\)\s*\{\s*\}',  # Empty step
        r'@\w+\([^)]*\)\s*public\s+void\s+\w+\([^)]*\)\s*\{\s*//[^\n]*\s*\}',  # Only comment
        r'@\w+\([^)]*\)\s*public\s+void\s+\w+\([^)]*\)\s*\{\s*throw\s+new\s+\w+',  # Throws exception
        r'@\w+\([^)]*\)\s*public\s+void\s+\w+\([^)]*\)\s*\{\s*System\.out',  # Only prints
        r'@\w+\([^)]*\)\s*public\s+void\s+\w+\([^)]*\)\s*\{\s*Assert\.',  # Only assertions
        r'@\w+\([^)]*\)\s*public\s+void\s+\w+\([^)]*\)\s*\{\s*[^}]{0,10}\s*\}',  # Very short implementations
        r'@\w+\([^)]*\)\s*public\s+void\s+\w+\([^)]*\)\s*\{\s*//\s*TODO[^}]*\}'  # TODO comments
    ]

    # Find all step definition files
    for pattern in step_def_paths:
            ...
            try:
                with open(file_path, 'r', encoding='utf-8') as f:
                    ...
                    for pattern in unimplemented_patterns:
                        ...
                        unimplemented_steps += len(matches)
                        if matches:
                            logger.debug(f"Found {len(matches)} unimplemented steps matching pattern: {pattern}")
            except Exception as e:
                logger.error(f"Error analyzing step definitions in {file_path}: {str(e)}")

    # Add unimplemented steps count to results
    results['step_definitions']['unimplemented_steps'] = unimplemented_steps
...
def analyze_selenium_implementation(project_path, results):
    ...
    actions_class_usage = False
    javascript_executor = False
    thread_sleep_usage = False
    screenshot_in_hooks = False
    webdriver_manager_usage = False

    # Patterns for analysis
    patterns = {
        'actions': [r'Actions\(', r'action\.', r'Actions\s+action', r'new\s+Actions'],
        'javascript': [r'JavascriptExecutor', r'executeScript', r'js\.'],
        'explicit_wait': [r'WebDriverWait', r'wait\.until', r'ExpectedConditions'],
        'custom_wait': [r'class.*?Wait', r'new\s+FluentWait', r'Wait<WebDriver>'],
        'thread_sleep': [r'Thread\.sleep', r'TimeUnit\.SECONDS\.sleep'],
        'pico_container': [
            r'import\s+org\.picocontainer',
            r'@Inject',
            r'PicoContainer',
            r'DefaultPicoContainer',
            r'@ScenarioScoped',
            r'@StepClassScoped'
        ],
        'screenshot': [
        ...
        'webdriver_manager': [r'WebDriverManager', r'ChromeDriverManager', r'FirefoxDriverManager']
    }

    # Check hooks for screenshots
    hooks_files = []
    for ext in ['.java']:
        hooks_files.extend(list(project_path.glob(f'**/Hooks{ext}')))
        hooks_files.extend(list(project_path.glob(f'**/Hook{ext}')))
        hooks_files.extend(list(project_path.glob('**/BaseHooks{ext}')))
        hooks_files.extend(list(project_path.glob('**/TestHooks{ext}')))
        # Also check files with @After annotations
        hooks_files.extend(list(project_path.glob('**/*Steps.java')))
        hooks_files.extend(list(project_path.glob('**/*Test.java')))
        hooks_files.extend(list(project_path.glob('**/Base*.java')))

    for file_path in hooks_files:
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
                # Check for @After with screenshot
                if '@After' in content and ('TakesScreenshot' in content or 'getScreenshotAs' in content):
                    screenshot_in_hooks = True
                    screenshot_capability = True
                    break
                # Check other screenshot patterns
                if any(re.search(pattern, content) for pattern in patterns['screenshot']):
                    screenshot_in_hooks = True
                    screenshot_capability = True
                    break
        except Exception as e:
            logger.error(f"Error analyzing hooks file {file_path}: {str(e)}")
    ...
    for file_path in code_files:
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                # Check Actions class usage
                if not actions_class_usage:
                    actions_class_usage = any(re.search(pattern, content) for pattern in patterns['actions'])

                # Check JavaScript executor
                if not javascript_executor:
                    javascript_executor = any(re.search(pattern, content) for pattern in patterns['javascript'])

                # Check explicit waits
                if not explicit_waits:
                    explicit_waits = any(re.search(pattern, content) for pattern in patterns['explicit_wait'])

                # Check custom wait conditions
                if not custom_wait_conditions:
                    custom_wait_conditions = any(re.search(pattern, content) for pattern in patterns['custom_wait'])

                # Check Thread.sleep usage
    ...
    # Calculate wait strategy score based on findings
    if custom_wait_conditions:
        wait_strategy = 100  # Best practice
    elif explicit_waits and not thread_sleep_usage:
        wait_strategy = 90   # Very good
    elif explicit_waits and thread_sleep_usage:
        wait_strategy = 70   # Good but with Thread.sleep
    elif thread_sleep_usage:
        wait_strategy = 30   # Not ideal
    else:
        wait_strategy = 50   # Default waits only

    # Check for PicoContainer implementation
    pico_container = False
    for file_path in code_files:
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                content = f.read()
                if any(re.search(pattern, content) for pattern in patterns['pico_container']):
                    pico_container = True
                    break
        except Exception as e:
            logger.error(f"Error checking PicoContainer in {file_path}: {str(e)}")
    ...
    results['selenium_implementation']['thread_sleep_usage'] = thread_sleep_usage
    results['selenium_implementation']['screenshot_in_hooks'] = screenshot_in_hooks
...
def analyze_page_objects(project_path, results):
    ...
    # Look for files in pageObject or pages directory
    for ext in ['.java']:
        page_object_files.extend(list(project_path.glob('**/pageObject/**/*' + ext)))
        page_object_files.extend(list(project_path.glob('**/pages/**/*' + ext)))
        page_object_files.extend(list(project_path.glob('**/pageObjects/**/*' + ext)))
        page_object_files.extend(list(project_path.glob('**/pageObjects/*' + ext)))
        # Add test/java/pageObjects pattern
        page_object_files.extend(list(project_path.glob('**/test/java/pageObjects/*' + ext)))
        # Add java/pageObjects pattern
        page_object_files.extend(list(project_path.glob('**/java/pageObjects/*' + ext)))
    ...
    base_page_pattern = False
    proper_encapsulation = False
    uniform_locators = True
    assertions_in_pages = False
    locator_types = set()
    
    # Patterns to check
    locator_patterns = {
        'by': r'By\.(id|name|xpath|cssSelector|className|tagName|linkText|partialLinkText)\s*\(',
        'find_by': r'@FindBy\s*\(',
        'assertions': r'(Assert\.|assertThat|assertEquals|assertTrue|assertFalse)'
    }
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                if re.search(r'class\s+\w+', content):
                    total_page_objects += 1
                    has_page_objects = True
                
                # Check locator uniformity
                by_locators = len(re.findall(r'By\.(id|name|xpath|cssSelector|className|tagName|linkText|partialLinkText)\s*\(', content))
                find_by_locators = len(re.findall(r'@FindBy\s*\(', content))
                
                if by_locators > 0:
                    locator_types.add('By')
                    total_page_objects += 1
                    has_page_objects = True
                if find_by_locators > 0:
                    locator_types.add('FindBy')
                    total_page_objects += 1 
                    has_page_objects = True
                
                # Check if this is a page object class
                if re.search(r'class.*Page', content):
                    total_page_objects += 1
                    has_page_objects = True
                
                # Check for assertions
                if re.search(locator_patterns['assertions'], content):
                    assertions_in_pages = True
                    
        except Exception as e:
            logger.error(f"Error analyzing page object file {file_path}: {str(e)}")
            
    # Check locator uniformity
    uniform_locators = len(locator_types) <= 1
            
    # Set results
    results['page_objects'].update({
        'has_page_objects': has_page_objects,
        'total_page_objects': total_page_objects,
        'base_page_pattern': base_page_pattern,
        'proper_encapsulation': proper_encapsulation,
        'uniform_locators': uniform_locators,
        'locator_types': list(locator_types),
        'assertions_in_pages': assertions_in_pages
    })
    ...
    if os.path.exists(pom_file):
        ...
        try:
            with open(pom_file, 'r', encoding='utf-8') as f:
                ...
                testng_dependency = False

                # Check for testng.xml file
                for root, _, files in os.walk(project_path):
                    if 'testng.xml' in files:
                        testng_xml_exists = True
                        break

                # Check for TestNG dependency
                if '<artifactId>testng</artifactId>' in pom_content:
                    testng_dependency = True

                # Mark TestNG config as present if both file and dependency exist
                enhancers['testng_config'] = testng_xml_exists and testng_dependency
