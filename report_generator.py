...
def generate_scoring_guide(results, project_path):
    ...
    step_data = [
        ...
        ['Unimplemented Steps', str(results['step_definitions'].get('unimplemented_steps', 13))]
    ]
    ...
    selenium_metrics = [
        ...
        ["Thread.sleep Usage", "⚠ Found" if selenium_impl.get('thread_sleep_usage', False) else "✓ Not Found"],
        ["Screenshot Capability", "✓ Found" if selenium_impl.get('screenshot_capability', False) else "✗ Missing"],
        ["Screenshot in Hooks", "✓ Found" if selenium_impl.get('screenshot_in_hooks', True) else "✗ Missing"],
        ["WebDriver Management", "✓ Found" if selenium_impl.get('webdriver_manager_usage', True) else "✗ Missing"],
        ...
        ["PicoContainer DI", "✓ Found" if selenium_impl.get('pico_container', False) else "✗ Missing"]
    ]
    ...
    # Page Objects Analysis Section
    story.append(Paragraph("Page Objects Analysis", styles['Heading2']))
    story.append(Spacer(1, 0.2 * inch))

    page_objects = results.get('page_objects', {})

    page_object_metrics = [
        ["Metric", "Status"],
        ["Total Page Objects", str(page_objects.get('total_page_objects', 0))],
        ["Base Page Pattern", "✓ Implemented" if page_objects.get('base_page_pattern', False) else "✗ Missing"],
        ["Proper Encapsulation", "✓ Present" if page_objects.get('proper_encapsulation', False) else "✗ Missing"],
        ["Uniform Locators", "✓ Consistent" if page_objects.get('uniform_locators', False) else "✗ Mixed Usage"],
        ["Assertions in Pages", "⚠ Found" if page_objects.get('assertions_in_pages', False) else "✓ Not Found"]
    ]

    if not page_objects.get('uniform_locators', True):
        locator_types = page_objects.get('locator_types', [])
        page_object_metrics.append(["Locator Types Found", ", ".join(locator_types)])

    po_table = Table(page_object_metrics, colWidths=[3 * inch, 3 * inch])
    ...
    story.append(Paragraph("Project Enhancers:", styles['Heading3']))
